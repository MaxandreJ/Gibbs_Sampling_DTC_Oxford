...
%   $Revision: 1.1.12.4.2.2 $  $Date: 2007/01/30 02:19:42 $
                ...
                case 1  % alphabet
                    ...
                    elseif ~strcmpi(pval,'nt') % If sequences are nucleotides
                        warning('Bioinfo:seqlogo:UnknownAlphabet',...
                            'Can not resolve alphabet type ''%s''. The results will be the default alphabet type nucleotides.',upper(pval));
                        NTFlag = true;
                    else
                        NTFlag = true;
                    end
...
if(isempty(strmatch('seqlogo.png', filename, 'exact')))
